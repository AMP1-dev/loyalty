...
export default function Cliente() {
  ...
  const isResgatandoRef = useRef(false);
  ...
  const resgatarBrinde = async (item: any) => {
    ...
    if (isResgatandoRef.current) return;
    const clean = cpf.replace(/\D/g, '');
    ...
    try {
      ...
      setCarregando(false);
      setTimeout(() => { isResgatandoRef.current = false; }, 500);
    }
  };
  ...
  const girarRoleta = () => {
    ...
    Animated.timing(rotateAnim, {
      ...
      let valorPontos = Number(win?.valor);
      if (isNaN(valorPontos) || !valorPontos) {
        const match = String(win?.nome || '').match(/\d+/);
        valorPontos = match ? Number(match[0]) : 10;
      }
      ...
      if (win?.tipo === 'pontos') {
         if (lid) {
            const { error: insErr } = await supabase.from('transacoes').insert([{ 
                ...
                loja_id: String(lid), 
                valor: 0, 
                pontos_gerados: valorPontos 
            }]);
            if (insErr) console.error("Erro insert roleta:", insErr);
         }
      } else if (win?.tipo === 'cashback') {
         if (lid) await supabase.from('cashbacks').insert([{ cliente_cpf: clean, loja_id: String(lid), valor: valorPontos }]);
      } else if (win?.tipo === 'brinde') {
         if (lid) await supabase.from('brindes_pendentes').insert([{ cliente_cpf: clean, loja_id: String(lid), nome_brinde: win.nome }]);
      }
      ...
      <View style={{ flex: 1, backgroundColor: c.bg }}>
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            >
              <Svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={c.texto} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
