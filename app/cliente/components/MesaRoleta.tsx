  ...
  const validarJogueDiario = async (telefone: string, lojaId: string): Promise<boolean> => {
    try {
      ...
      if (jaJogouLocal) {
        ...
        const hoje = new Date();
        if (dataLocal.getDate() === hoje.getDate() && dataLocal.getMonth() === hoje.getMonth() && dataLocal.getFullYear() === hoje.getFullYear()) return false;
      }
      const agora = new Date();
      const inicioHoje = new Date(agora.getFullYear(), agora.getMonth(), agora.getDate()).toISOString();
      const { data, error } = await supabase.from('roleta_mesa_participacoes').select('id').eq('loja_id', lojaId).eq('cliente_cpf', cleanTel).gte('created_at', inicioHoje).limit(1);
  ...
  const girarRoleta = async () => {
    ...
    try {
      ...
      Animated.timing(rotateAnim, {
        ...
        const cleanTel = telefone.replace(/\D/g, '');
        salvarParticipacaoMesa(cleanTel, premio).catch(e => console.error(e));
          ...
          {etapa === 'roleta' && (
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', width: '100%' }}>
              ...
              <TouchableOpacity onPress={girarRoleta} disabled={rodando} style={{ backgroundColor: rodando ? '#ccc' : c.roxo, borderRadius: 16, paddingVertical: 22, paddingHorizontal: 50, alignItems: 'center', marginTop: 40, elevation: 10 }}>
                <Text style={{ color: '#fff', fontSize: 20, fontWeight: '900' }}>{rodando ? 'GIRANDO...' : '🎰 GIRAR AGORA!'}</Text>
              </TouchableOpacity>
            </View>
          )}
              ...
              <View style={{ alignItems: 'center', width: '90%', padding: 20 }}>
                ...
                <View style={{ backgroundColor: c.card, borderRadius: 24, padding: 30, marginTop: 30, borderWidth: 1, borderColor: c.borda, width: '100%', alignItems: 'center', elevation: 5 }}>
                  <Text style={{ fontSize: 14, color: c.subtexto, textAlign: 'center', lineHeight: 20 }}>
                    {(!premioGanho.nome.toLowerCase().includes('tente') && premioGanho.tipo !== 'nada') ? 'Mostre esta tela para o atendente e retire seu prêmio agora mesmo! 🍀' : 'Não foi dessa vez, mas não desista! Amanhã tem mais sorte pra você. ✨'}
                  </Text>
                </View>
                <TouchableOpacity onPress={() => { setEtapa('telefone'); setTelefone(''); setNotaNps(0); setPremioGanho(null); rotateAnim.setValue(0); }} style={{ backgroundColor: c.roxo, borderRadius: 20, paddingVertical: 20, paddingHorizontal: 60, alignItems: 'center', marginTop: 40, width: '100%', elevation: 10 }}>
